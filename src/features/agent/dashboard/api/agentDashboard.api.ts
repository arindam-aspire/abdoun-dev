...
export async function fetchAgentDashboardData(): Promise<AgentDashboardData> {
  try {
    const response = await authApi.get<AgentDashboardSummaryApiData>(
      "/agents/dashboard/summary",
    );
    return toAgentDashboardData((response.data ?? {}) as AgentDashboardSummaryApiData);
  } catch (error) {
...
export type AgentPropertyPerformanceResult = PaginatedResult<PerformanceComparisonItem>;
...
function parseAgentPropertyPerformancePayload(data: AgentPropertyPerformanceApiData): AgentPropertyPerformanceResult {
  ...
  if (Array.isArray(data)) {
    ...
    return createPaginatedResult(items, undefined, { total: items.length });
  }
  ...
  if (data && typeof data === "object") {
    ...
    return createPaginatedResult(items, o, { total: Math.max(total, items.length) });
  }

  return createPaginatedResult([], undefined, { total: 0 });
}
...
export async function fetchAgentPropertyPerformance(
  ...
  try {
    const response = await authApi.get<AgentPropertyPerformanceApiData>(
      "/agent/property-performance",
      {
        params: { page, pageSize, period },
      },
    );
    return parseAgentPropertyPerformancePayload(response.data);
  } catch (error) {
    throw new Error(getApiErrorMessage(error), { cause: error });
  }
}
