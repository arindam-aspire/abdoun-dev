...
export interface AgentSearchResultListCardProps {
  ...
  editBasePath?: string;
  onDelete?: (listing: SearchResultListing) => void;
  actionLabels?: { edit?: string; delete?: string };
}

export function AgentSearchResultListCard({
  listing,
  translations: t,
  detailsBasePath = "agent-properties",
  editBasePath = "agent-properties",
  onDelete,
  actionLabels = {},
}: AgentSearchResultListCardProps) {
  const { edit: editLabel = "Edit", delete: deleteLabel = "Delete" } = actionLabels;
  ...
  const editHref = `/${locale}/${editBasePath}/${listing.id}/edit`;
  ...
  const showVerified = Array.isArray(listing.badges) && listing.badges.includes("Verified");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={cn("absolute top-2 z-10 flex flex-wrap items-center gap-1.5", isRtl ? "right-2" : "left-2")}>
          {showExclusive && (
        ...
        <div ref={menuRef} className={cn("absolute top-2 z-10", isRtl ? "left-2" : "right-2")}>
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault();
            ...
            {images.map((_, i) => (
              <span
                ...
                className={cn(
                  ...
                  i === currentImageIndex ? "bg-white ring-2 ring-white/50" : "bg-white/60",
                )}
                aria-hidden
              />
            ))}
        ...
        <div className={cn("absolute bottom-8 left-1/2 z-0 -translate-x-1/2 text-size-2xs fw-medium text-white/70", isRtl && "translate-x-1/2")}>
          {listing.brokerName}
        </div>
      </Link>

      <div className={cn("flex min-w-0 flex-1 flex-col justify-between p-4 md:p-5", isRtl ? "text-right" : "text-left")}>
        <div>
          <Link
            href={detailsHref}
            {...linkProps}
          ...
          >
            <h3 className="text-size-xl fw-bold leading-tight text-charcoal">{listing.title}</h3>
            {developerName && <p className="mt-0.5 text-size-sm text-charcoal/80">{t.byDeveloper(developerName)}</p>}
          </Link>

          <div className={cn("mt-3 flex flex-wrap gap-2", isRtl && "flex-row-reverse justify-end")}>
            <div className="rounded-lg bg-charcoal-5 px-3 py-2 min-w-0">
              <p className="text-size-2xs fw-semibold uppercase tracking-wider text-charcoal/60">{t.launchPrice}</p>
              <p className="text-size-sm fw-bold text-charcoal">{listing.price}</p>
            </div>
            {listing.paymentPlan && (
              <div className="rounded-lg bg-charcoal-5 px-3 py-2 min-w-0">
                <p className="text-size-2xs fw-semibold uppercase tracking-wider text-charcoal/60">{t.paymentPlan}</p>
            ...
            {listing.handover && (
              <div className="rounded-lg bg-charcoal-5 px-3 py-2 min-w-0">
                <p className="text-size-2xs fw-semibold uppercase tracking-wider text-charcoal/60">{t.handover}</p>
                <p className="text-size-sm fw-bold text-charcoal">{listing.handover}</p>
              </div>
            )}
          </div>

          <p className="mt-2 text-size-sm text-charcoal">{listing.searchPropertyType ?? listing.propertyType}</p>

          {(listing.area || listing.acres) && (
            <p className="mt-0.5 text-size-sm text-charcoal/80">{listing.acres ? `${listing.acres} acres` : `${listing.area} sqft`}</p>
          )}

          <div className={cn("mt-1 flex items-start gap-1.5 text-size-sm text-charcoal/90", isRtl && "flex-row-reverse justify-end")}>
            <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-primary" aria-hidden />
            <span className="line-clamp-2">{listing.location}</span>
          </div>
        </div>

        <div className={cn("mt-4 flex flex-col gap-3 border-t border-subtle pt-4", isRtl && "items-end")}>
          <div className={cn("grid grid-cols-3 gap-1.5", isRtl && "flex-row-reverse")}>
            <button
              type="button"
              onClick={(e) => {
                e.preventDefault();
          ...
          {developerName && (
            <div className={cn("rounded-lg bg-charcoal-8 px-3 py-2 text-size-sm fw-medium text-charcoal", isRtl && "mr-0")}>
