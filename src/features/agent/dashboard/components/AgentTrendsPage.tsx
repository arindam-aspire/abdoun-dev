...
import { aggregateInquiryDailyToFourWeeks } from "@/features/agent/dashboard/utils/aggregateInquiryDailyToFourWeeks";
import { AgentTrendsPageSkeleton } from "@/features/agent/dashboard/components/AgentTrendsPageSkeleton";
...
export function AgentTrendsPage() {
  ...
  useEffect(() => {
    ...
    const message = error ?? t("loadDashboardError");
  ...
  if (error || !data) {
    const fallbackErrorMessage = error ?? t("loadDashboardError");
