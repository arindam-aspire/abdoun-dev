...
import {
  ...
  COMMERCIAL_FLOOR_LEVEL_TYPES,
  COMMERCIAL_LOADING_ACCESS_TYPES,
  COMMERCIAL_STORAGE_AREA_TYPES,
  dropdownPanelClass,
  FLOOR_OPTIONS,
  fromCamelCase,
  LAND_ELECTRICITY_TYPES,
  LAND_UTILITIES_TYPES,
  LAND_WATER_SOURCE_TYPES,
  LAND_ZONED_USE_TYPES,
  PARKING_OPTIONS,
  PROPERTY_AGE_OPTIONS,
  PROPERTY_TYPE_PLACEHOLDER,
  RESIDENTIAL_BALCONY_TYPES,
  RESIDENTIAL_CLOSET_TYPES,
  RESIDENTIAL_FLOOR_LEVEL_TYPES,
  RESIDENTIAL_FURNITURE_TYPES,
  RESIDENTIAL_GARDEN_TYPES,
  RESIDENTIAL_GYM_ACCESS_TYPES,
...
export function SearchFields({
  ...
  const getInitialBudgetMin = (): string => {
    const canonical = searchParams.get("budgetMin")?.trim();
    if (canonical) return canonical;
    const legacy = searchParams.get("minPrice")?.trim();
    return legacy ?? "";
  };

  const getInitialBudgetMax = (): string => {
    const canonical = searchParams.get("budgetMax")?.trim();
    if (canonical) return canonical;
    const legacy = searchParams.get("maxPrice")?.trim();
    return legacy ?? "";
  };
