...
type PropertySearchApiResponse = {
  /** Preferred list key (avoids `data.data` with the v1 envelope). */
  items?: PropertySearchApiItem[] | null;
  total: number;
  page: number;
  pageSize: number;
  /** Legacy inner field; supported until the API stops sending it. */
  data?: PropertySearchApiItem[] | null;
};

function propertySearchRows(search: PropertySearchApiResponse): PropertySearchApiItem[] {
  if (Array.isArray(search.items)) return search.items;
  if (Array.isArray(search.data)) return search.data;
  return [];
}

export type PropertySearchResult = PaginatedResult<SearchResultListing>;

export type ExclusivePropertiesResult = PaginatedResult<HomeProperty>;
...
export async function fetchPropertiesByQuery(
  ...
  return createPaginatedResult(propertySearchRows(search).map(toListing), search, {
    page: search.page,
...
export async function fetchExclusiveProperties(
  ...
  const response = await publicApi.get<PropertySearchApiResponse>("/properties/exclusive", {
    params: {
      page: 1,
      pageSize,
    },
  ...
  const search = response.data;
  return createPaginatedResult(propertySearchRows(search).map(toHomeProperty), search, {
    page: search.page,
    pageSize: search.pageSize,
    total: search.total,
  });
...
function similarPropertyRows(
  payload: SimilarPropertiesApiResponse,
): PropertySearchApiItem[] {
...
export async function fetchSimilarPropertiesById(
  ...
  return similarPropertyRows(response.data).map(toListing);
}
