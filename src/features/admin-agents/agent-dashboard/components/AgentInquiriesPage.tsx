...
export function AgentInquiriesPage() {
  ...
  const modalStatusOptions = isDealCloseView
    ? DEAL_CLOSE_FILTERS
    : LEADS_FILTERS.filter((status) => status !== "all");
  ...
  return (
    <div className="space-y-6">
      ...
      <DialogRoot open={!!selectedId} onClose={closeModal}>
        ...
        <DialogDescription>{selected ? selected.propertyName : t("loading")}</DialogDescription>
