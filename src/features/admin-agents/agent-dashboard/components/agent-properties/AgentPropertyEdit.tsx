...
export function AgentPropertyEdit({ language, propertyId }: AgentPropertyEditProps) {
  const isRtl = language === "ar";
  const [title, setTitle] = useState(MOCK_AGENT_PROPERTY.title);
  const [subtitle, setSubtitle] = useState(MOCK_AGENT_PROPERTY.subtitle ?? "");
  const [price, setPrice] = useState(MOCK_AGENT_PROPERTY.price);
  const [beds, setBeds] = useState(String(MOCK_AGENT_PROPERTY.beds ?? 0));
  const [baths, setBaths] = useState(String(MOCK_AGENT_PROPERTY.baths ?? 0));
  const [area, setArea] = useState(MOCK_AGENT_PROPERTY.area ?? "");
  const [status, setStatus] = useState(MOCK_AGENT_PROPERTY.status ?? "");
  const [location, setLocation] = useState(MOCK_AGENT_PROPERTY.location ?? "");
  const [heroImage, setHeroImage] = useState(MOCK_AGENT_PROPERTY.image ?? "");
  const [galleryImages, setGalleryImages] = useState<string[]>(MOCK_AGENT_PROPERTY.gallery ?? []);
  const [description, setDescription] = useState(MOCK_AGENT_PROPERTY.description ?? "");
  const [amenitiesText, setAmenitiesText] = useState((MOCK_AGENT_PROPERTY.amenities ?? []).join("\n"));
  const [virtualTourUrl, setVirtualTourUrl] = useState(MOCK_AGENT_PROPERTY.virtualTourUrl ?? "");
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-subtle md:p-6">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-lg font-semibold text-charcoal">Agent property editor</h1>
              <p className="mt-1 text-size-sm text-charcoal/70">
                See exactly how buyers will experience this listing while you fine-tune the details below.
              </p>
            </div>
          ...
          <div className="mt-4 relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/15 via-secondary/10 to-accent/15 p-[1px] shadow-sm ring-1 ring-subtle/60">
            <div className="h-full rounded-2xl bg-white/95 p-3 md:p-4">
              <div className="max-h-[560px] overflow-y-auto rounded-xl bg-surface/40 p-2 md:p-3">
                ...
                {(previewProperty.beds || previewProperty.baths || previewProperty.area) && (
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div className="grid gap-4 md:grid-cols-2 md:items-start">
              <div className="space-y-4">
                <div>
                  <p className="text-size-xs fw-semibold uppercase tracking-wide text-charcoal/60">Basic info</p>
                </div>
                <div>
                  <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Title</label>
                  <input
                    type="text"
                    value={title}
                ...
                <div>
                  <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Subtitle</label>
                  <input
                    type="text"
                    value={subtitle}
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-size-xs fw-semibold uppercase tracking-wide text-charcoal/60">Pricing & stats</p>
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Price</label>
                    <input
                      type="text"
                      value={price}
                  ...
                  <div>
                    <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Status label</label>
                    <input
                      type="text"
                      value={status}
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div>
                    <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Bedrooms</label>
                    <input
                      type="number"
                      min={0}
                      value={beds}
                  ...
                  <div>
                    <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Bathrooms</label>
                    <input
                      type="number"
                      min={0}
                      value={baths}
                  ...
                  <div>
                    <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Area (sqft)</label>
                    <input
                      type="text"
                      value={area}
            ...
            <div className="grid gap-4 md:grid-cols-2 md:items-start">
              <div className="space-y-3">
                <p className="text-size-xs fw-semibold uppercase tracking-wide text-charcoal/60">Media & location</p>
                <div>
                  <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Images</label>
                  <input
                    type="file"
                    accept="image/*"
                    multiple
                  ...
                  <p className="mt-1 text-size-2xs text-charcoal/50">
                    Demo upload only. Images are used for preview and are not persisted.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Location</label>
                  <input
                    type="text"
                    value={location}
                ...
                <div>
                  ...
                  />
                  <p className="mt-1 text-size-2xs text-charcoal/50">
                    Paste a link (e.g. https://my.matterport.com/show/?m=...) or the full iframe code.
                    The tour is embedded on the property page; leave empty to hide.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1 block text-size-xs fw-medium text-charcoal/80">Description</label>
                <textarea
                  value={description}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="min-h-[96px] w-full rounded-lg border border-subtle bg-surface/40 px-3 py-2 text-size-sm text-charcoal placeholder:text-charcoal/50 focus:border-secondary focus:outline-none"
                  placeholder={"Open-plan living and dining\nCovered balcony\nAllocated parking space"}
                />
              </div>
            </div>

            {savedMessage && <p className="text-size-xs text-emerald-600">{savedMessage}</p>}
