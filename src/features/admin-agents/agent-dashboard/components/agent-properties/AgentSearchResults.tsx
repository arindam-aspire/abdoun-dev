...
export function AgentSearchResults({ language, resultsTitle, searchParams }: AgentSearchResultsProps) {
        ...
        {totalItems === 0 ? (
          ...
          listings.map((listing) => (
            <li key={listing.id} className="min-h-0">
              {view === "list" ? (
                <AgentSearchResultListCard listing={listing} translations={listCardTranslations} />
              ) : (
                <AgentSearchResultGridCard listing={listing} translations={cardTranslations} />
              )}
            </li>
          ))
        )}
