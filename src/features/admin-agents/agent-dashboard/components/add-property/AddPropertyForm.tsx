...
import type { CategoryKey, StatusTabKey } from "@/features/property-search/types";
...
import { DocumentUploadField, createUploadedFiles, type UploadedFile } from "./DocumentUploadField";
...
export function AddPropertyForm() {
  ...
  const showFloorLevel = isResType(RESIDENTIAL_FLOOR_LEVEL_TYPES) || isComType(COMMERCIAL_FLOOR_LEVEL_TYPES);
  ...
  const cityOptions = JORDAN_CITIES_WITH_AREAS.map((c) => ({ value: c.name, label: c.name }));
  const areaOptions = city ? getAreasByCityName(city).map((a) => ({ value: a, label: a })) : [];
  const propertyTypeOptions = CATEGORY_TO_PROPERTY_TYPES[category].map((pt) => ({ value: pt, label: pt }));
  ...
  const updateOwner = useCallback(
    <K extends keyof Omit<OwnerFormState, "id">>(
      ...
      setOwners((prev) =>
        prev.map((owner) => (owner.id === ownerId ? { ...owner, [field]: value } : owner)),
      );
    },
    [],
  );
  ...
  return (
    <div className="space-y-6 pb-28">
      ...
      <div className="px-1">
        ...
        <p className="mt-1 text-size-sm text-charcoal/70">{t("addPropertyPageSubtitle")}</p>
      </div>

      <PropertyFormSection icon={<Tag className="h-5 w-5" />} title={t("sectionBasicInfo")} subtitle={t("sectionBasicInfoHint")}>
      ...
      <PropertyFormSection icon={<MapPin className="h-5 w-5" />} title={t("sectionLocation")} subtitle={t("sectionLocationHint")}>
        <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="space-y-4 rounded-2xl border border-subtle bg-surface/30 p-4">
          <div className="space-y-1">
            <h4 className="text-size-sm fw-semibold text-charcoal">{t("exactLocationTitle")}</h4>
            <p className="text-size-xs text-charcoal/65">{t("exactLocationHint")}</p>
          </div>

          <LocationPicker city={city} area={area} address={address} onLocationSelect={setExactLocation} />
        </div>
      ...
      <PropertyFormSection icon={<Building2 className="h-5 w-5" />} title={t("sectionDetails")} subtitle={t("sectionDetailsHint")}>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {showBedrooms && (
          ...
          {completionStatus !== "ready" && (
            <PropertyFormField label={t("fieldHandoverDate")} htmlFor="handover-date">
              <Input id="handover-date" type="date" value={handoverDate} onChange={(e) => setHandoverDate(e.target.value)} />
            </PropertyFormField>
          )}
      ...
      <PropertyFormSection icon={<DollarSign className="h-5 w-5" />} title={t("sectionPricing")} subtitle={t("sectionPricingHint")}>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {isRental ? (
            <>
      ...
      {amenityItems.length > 0 && (
        <PropertyFormSection icon={<Sparkles className="h-5 w-5" />} title={t("sectionFeatures")} subtitle={t("sectionFeaturesHint")}>
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3 lg:grid-cols-4">
            {amenityItems.map((item) => (
              <label key={item.key} className="flex items-center gap-2.5 cursor-pointer group">
                <Checkbox checked={item.checked} onChange={(e) => item.set((e.target as HTMLInputElement).checked)} />
      ...
      <PropertyFormSection icon={<ImageIcon className="h-5 w-5" />} title={t("sectionMedia")} subtitle={t("sectionMediaHint")}>
      ...
      <PropertyFormSection icon={<FileText className="h-5 w-5" />} title={t("sectionDocuments")} subtitle={t("sectionDocumentsHint")}>
