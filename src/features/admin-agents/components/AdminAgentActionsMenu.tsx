...
export function AdminAgentActionsMenu({ agent, adminId, onToast }: AdminAgentActionsMenuProps) {
  ...
  const run = useCallback(
    async (key: string, fn: () => Promise<unknown>, successMessage: string) => {
      ...
      try {
        ...
        const detail =
          anyError.response?.data?.detail ?? anyError.response?.data?.message ?? anyError.message;

        const message = typeof detail === "string" && detail.trim() ? detail : "Action failed.";
  ...
  const items = useMemo(() => {
    ...
    if (agent.status === AGENT_STATUS.INVITED) {
      list.push({
        ...
        onSelect: () => setPendingAction("resend-invite"),
      });
      list.push({
        key: "revoke-invite",
        ...
        onSelect: () => setPendingAction("revoke-invite"),
      });
      ...
      list.push({
        ...
        onSelect: () => setPendingAction(isActive ? "set-inactive" : "set-active"),
      });
    }
  ...
  const handleConfirm = async () => {
    ...
    if (!agentId) {
      onToast({ kind: "error", message: "Agent id is missing. Please refresh and try again." });
      resetPending();
      return;
    }
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        cancelButtonClassName={activeConfirm?.cancelButtonClassName ?? "text-charcoal/80 hover:bg-charcoal/5"}
