    ...
    >
      ...
      >
        ...
        <div className={cn("absolute top-2 z-10 flex flex-wrap items-center gap-1.5", isRtl ? "right-2" : "left-2")}>
          {showExclusive && (
            <Badge className="bg-accent text-secondary fw-semibold" variant="secondary">
              Exclusive
            </Badge>
          )}
          {showVerified && (
        ...
        <div ref={menuRef} className={cn("absolute top-2 z-10", isRtl ? "left-2" : "right-2")}>
          ...
          {menuOpen && (
            <div className={cn("absolute top-11 z-50 min-w-[140px] rounded-lg border border-subtle bg-white py-1 shadow-xl", isRtl ? "left-0" : "right-0")}>
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault();
        ...
        {totalImages > 1 && (
          ...
          >
            {images.map((_, i) => (
              <span
                key={i}
                className={cn("h-1.5 w-1.5 rounded-full transition", i === currentImageIndex ? "bg-white ring-2 ring-white/50" : "bg-white/60")}
                aria-hidden
              />
            ))}
          </div>
