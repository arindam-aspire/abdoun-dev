...
export function AdminSearchResultPropertyCard({
  ...
  return (
    <article
      className={cn("group relative flex h-full flex-col overflow-hidden rounded-xl border border-subtle bg-white shadow-sm transition hover:shadow-md")}
      dir={isRtl ? "rtl" : "ltr"}
    >
      <Link
        href={detailsHref}
      ...
      >
        ...
        {totalImages > 1 && (
          <div className={cn("absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 gap-1.5", isRtl && "flex-row-reverse")}>
            {images.map((_, i) => (
              <span
                key={i}
      ...
      <div className={cn("relative z-10 flex min-h-[120px] flex-1 flex-col justify-between p-3", isRtl ? "text-right" : "text-left")}>
        <Link
          href={detailsHref}
          {...linkProps}
        ...
        >
          <p className="text-size-base fw-bold leading-tight text-charcoal">{listing.price}</p>
          <p className="mt-0.5 text-size-xs text-charcoal/80 line-clamp-1">
            {listing.acres ? `${listing.acres} acres - ${listing.propertyType}` : `${listing.area} sqft - ${listing.propertyType}`}
          </p>
          <div className={cn("mt-1 flex items-center gap-1 text-size-11 text-charcoal/75", isRtl && "flex-row-reverse justify-end")}>
            <MapPin className="h-3 w-3 shrink-0 text-primary" aria-hidden />
            <span className="truncate">{listing.location}</span>
          </div>
          <p className="mt-1 text-size-2xs fw-semibold uppercase tracking-wider text-charcoal/60">{listing.brokerName}</p>
        </Link>

        <div className="mt-2 grid grid-cols-3 gap-1.5 border-t border-subtle pt-2">
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault();
