...
import { SearchResultSortDropdown } from "@/features/property-search/components/SearchResultSortDropdown";
import { SearchResultViewToggle } from "@/features/property-search/components/SearchResultViewToggle";
import { Pagination } from "@/components/ui/Pagination";
import { MOCK_SEARCH_RESULTS } from "@/lib/mocks/mockSearchResults";
import type { CategoryKey, SearchResultListing, StatusTabKey } from "@/features/property-search/types";
...
function getSortParam(searchParams: URLSearchParams): SortKey {
  ...
  if (sort === "newest" || sort === "price_asc" || sort === "price_desc") return sort;
  return "newest";
}
...
export function AdminSearchResults({ resultsTitle }: AdminSearchResultsProps) {
  ...
  const cardTranslations = { email: t("email"), call: t("call") };
    ...
    <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-[var(--border-subtle)] md:p-5">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <SearchResultSortDropdown value={sort} onSelect={setSort} getLabel={(key) => t(key)} />
      ...
      >
        {totalItems === 0 ? (
          <li className="col-span-full py-12 text-center text-[var(--color-charcoal)]/80">{t("noResults")}</li>
        ) : (
          listings.map((listing) => (
            <li key={listing.id} className="min-h-0">
              {view === "list" ? (
                <AdminSearchResultListCard listing={listing} translations={listCardTranslations} />
              ) : (
                <AdminSearchResultPropertyCard listing={listing} translations={cardTranslations} />
              )}
            </li>
          ))
        )}
