...
export function AdminUserGrowthDetailsPageSkeleton() {
  return (
    <div className="space-y-6" aria-busy="true" aria-label="Loading user growth trends">
      <Skeleton className="h-4 w-40 rounded-md" />

      <div className="space-y-2 px-1">
        <Skeleton className="h-9 w-[min(100%,20rem)] max-w-md rounded-md" />
      ...
      <section className="rounded-2xl border border-subtle bg-white p-4 shadow-sm md:p-5">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="rounded-xl border border-subtle bg-surface px-3 py-3 space-y-2"
            >
