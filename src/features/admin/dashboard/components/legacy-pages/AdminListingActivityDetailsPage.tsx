...
import { AdminListingActivityDetailsPageSkeleton } from "@/features/admin/dashboard/components/AdminListingActivityDetailsPageSkeleton";
...
export function AdminListingActivityDetailsPage() {
  ...
  const values = data?.listingGrowthSeries ?? [];
  ...
  if (loading) {
    return <AdminListingActivityDetailsPageSkeleton />;
  }

  if (status === "failed" || !data) {
    const fallbackMessage = error ?? "Unable to load listing activity.";
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="px-1">
        ...
        <p className="mt-1 text-size-sm text-charcoal/70">
          Monthly new listings created over the last 12 months.
        </p>
      </div>
          ...
          <table className="min-w-[520px] w-full border-collapse">
            <thead>
              <tr className="border-b border-subtle text-left text-xs text-charcoal/70">
                ...
                <th className="py-2 pr-4 font-medium">New listings</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => {
