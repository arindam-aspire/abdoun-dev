...
export interface AgentSearchResultGridCardProps {
  ...
  /** Base path segment for details page, default: "agent-properties" */
  detailsBasePath?: string;
  /** Base path segment for edit page, default: "agent-properties" */
  editBasePath?: string;
  /** Optional: called when agent clicks Delete (e.g. to show confirm modal or call API) */
  onDelete?: (listing: SearchResultListing) => void;
  /** Optional labels for menu actions; defaults: Edit, Delete */
...
export function AgentSearchResultGridCard({
  ...
  const { edit: editLabel = "Edit", delete: deleteLabel = "Delete" } =
    actionLabels;
  ...
  return (
    ...
    >
      ...
      <EmailAgentModal
        key={`${listing.id}-${signedInUser?.id ?? "guest"}-${
          emailModalOpen ? "open" : "closed"
        }`}
        open={emailModalOpen}
