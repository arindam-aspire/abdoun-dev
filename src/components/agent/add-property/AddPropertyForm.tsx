...
import { CATEGORY_TO_PROPERTY_TYPES } from "@/components/search-result/types";
import type { CategoryKey, StatusTabKey } from "@/components/search-result/types";
import {
...
/* ------------------------------------------------------------------ */
/*  Constants – same visibility rules as SearchFields                  */
/* ------------------------------------------------------------------ */
...
const COMMERCIAL_AC_TYPES = new Set(["Offices", "Ready Businesses", "Shops", "Showrooms"]);
const COMMERCIAL_STORAGE_AREA_TYPES = new Set(["Warehouses"]);
const LAND_UTILITIES_TYPES = new Set(["Commercial Lands", "Industrial Lands", "Mixed Use Lands", "Residential Lands"]);
const LAND_ZONED_USE_TYPES = new Set(["Commercial Lands", "Industrial Lands", "Mixed Use Lands"]);
const LAND_WATER_SOURCE_TYPES = new Set(["Agricultural Lands"]);
const LAND_ELECTRICITY_TYPES = new Set(["Commercial Lands", "Industrial Lands", "Mixed Use Lands", "Residential Lands"]);

/* Additional visibility sets */
...
const COMMERCIAL_CCTV_TYPES = new Set(["Warehouses", "Shops", "Showrooms", "Offices"]);
const LAND_FENCED_TYPES = new Set(["Agricultural Lands", "Residential Lands", "Industrial Lands"]);
...
const ORIENTATION_OPTIONS = ["North", "South", "East", "West", "North-East", "North-West", "South-East", "South-West"];
...
/** Map category-level property type labels to the agent PropertyType enum for saving */
function mapToAgentPropertyType(category: CategoryKey, propertyType: string): PropertyType {
...
export function AddPropertyForm() {
  ...
  // ---- Rental (when purpose = rent): single price based on frequency ----
  ...
  // ---- Derived visibility flags (mirror SearchFields logic) ----
  ...
  const showFloorLevel =
    isResType(RESIDENTIAL_FLOOR_LEVEL_TYPES) || isComType(COMMERCIAL_FLOOR_LEVEL_TYPES);
  ...
  const amenityItems = useMemo(() => {
    const items: { key: string; label: string; checked: boolean; set: (v: boolean) => void }[] = [];
    if (isResType(RESIDENTIAL_BALCONY_TYPES))
      items.push({ key: "balcony", label: t("amenityBalcony"), checked: balcony, set: setBalcony });
    if (isResType(RESIDENTIAL_CLOSET_TYPES))
      items.push({ key: "builtInCloset", label: t("amenityBuiltInCloset"), checked: builtInCloset, set: setBuiltInCloset });
    if (isResType(RESIDENTIAL_GARDEN_TYPES))
      items.push({ key: "garden", label: t("amenityGarden"), checked: garden, set: setGarden });
    if (category === "residential" || category === "commercial")
      items.push({ key: "alarmSystem", label: t("amenityAlarmSystem"), checked: alarmSystem, set: setAlarmSystem });
    if (isResType(RESIDENTIAL_HOME_AUTOMATION_TYPES))
      items.push({ key: "homeAutomation", label: t("amenityHomeAutomation"), checked: homeAutomation, set: setHomeAutomation });
    if (isResType(RESIDENTIAL_GYM_ACCESS_TYPES))
      items.push({ key: "gymAccess", label: t("amenityGymAccess"), checked: gymAccess, set: setGymAccess });
    if (category === "residential" || category === "commercial")
      items.push({ key: "parkingAvailable", label: t("amenityParkingAvailable"), checked: parkingAvailable, set: setParkingAvailable });
    if (category === "residential" || category === "commercial")
      items.push({ key: "swimmingPool", label: t("amenitySwimmingPool"), checked: swimmingPool, set: setSwimmingPool });
    if (isComType(COMMERCIAL_LOADING_ACCESS_TYPES))
      items.push({ key: "loadingAccess", label: t("amenityLoadingAccess"), checked: loadingAccess, set: setLoadingAccess });
    if (isComType(COMMERCIAL_DISPLAY_FRONTAGE_TYPES))
      items.push({ key: "displayFrontage", label: t("amenityDisplayFrontage"), checked: displayFrontage, set: setDisplayFrontage });
    if (isComType(COMMERCIAL_AC_TYPES))
      items.push({ key: "airConditioning", label: t("amenityAirConditioning"), checked: airConditioning, set: setAirConditioning });
    if (isComType(COMMERCIAL_STORAGE_AREA_TYPES))
      items.push({ key: "storageArea", label: t("amenityStorageArea"), checked: storageArea, set: setStorageArea });
    if (category === "land")
      items.push({ key: "roadAccess", label: t("amenityRoadAccess"), checked: roadAccess, set: setRoadAccess });
    if (category === "land" && hasPropertyType && LAND_UTILITIES_TYPES.has(propertyType))
      items.push({ key: "utilitiesAvailable", label: t("amenityUtilities"), checked: utilitiesAvailable, set: setUtilitiesAvailable });
    if (category === "land" && hasPropertyType && LAND_ZONED_USE_TYPES.has(propertyType))
      items.push({ key: "zonedUse", label: t("amenityZonedUse"), checked: zonedUse, set: setZonedUse });
    if (category === "land" && hasPropertyType && LAND_WATER_SOURCE_TYPES.has(propertyType))
      items.push({ key: "waterSource", label: t("amenityWaterSource"), checked: waterSource, set: setWaterSource });
    if (category === "land" && hasPropertyType && LAND_ELECTRICITY_TYPES.has(propertyType))
      items.push({ key: "electricityNearby", label: t("amenityElectricity"), checked: electricityNearby, set: setElectricityNearby });
    // Additional amenities
    if (isResType(RESIDENTIAL_MAID_ROOM_TYPES))
      items.push({ key: "maidsRoom", label: t("amenityMaidsRoom"), checked: maidsRoom, set: setMaidsRoom });
    if (isResType(RESIDENTIAL_STUDY_TYPES))
      items.push({ key: "studyRoom", label: t("amenityStudyRoom"), checked: studyRoom, set: setStudyRoom });
    if (isResType(RESIDENTIAL_LAUNDRY_TYPES))
      items.push({ key: "laundryRoom", label: t("amenityLaundryRoom"), checked: laundryRoom, set: setLaundryRoom });
    if (isResType(RESIDENTIAL_CENTRAL_AC_TYPES))
      items.push({ key: "centralAC", label: t("amenityCentralAC"), checked: centralAC, set: setCentralAC });
    if (isResType(RESIDENTIAL_ELEVATOR_TYPES))
      items.push({ key: "elevator", label: t("amenityElevator"), checked: elevator, set: setElevator });
    if (isComType(COMMERCIAL_MEETING_ROOM_TYPES))
      items.push({ key: "meetingRoom", label: t("amenityMeetingRoom"), checked: meetingRoom, set: setMeetingRoom });
    if (isComType(COMMERCIAL_RECEPTION_TYPES))
      items.push({ key: "receptionArea", label: t("amenityReceptionArea"), checked: receptionArea, set: setReceptionArea });
    if (isComType(COMMERCIAL_CCTV_TYPES))
      items.push({ key: "cctvSecurity", label: t("amenityCctv"), checked: cctvSecurity, set: setCctvSecurity });
    if (category === "land" && hasPropertyType && LAND_FENCED_TYPES.has(propertyType))
      items.push({ key: "fencedBoundary", label: t("amenityFenced"), checked: fencedBoundary, set: setFencedBoundary });
    if (category === "land" && hasPropertyType && LAND_IRRIGATION_TYPES.has(propertyType))
      items.push({ key: "irrigationSystem", label: t("amenityIrrigation"), checked: irrigationSystem, set: setIrrigationSystem });
    // Universal amenities
    if (category === "residential")
      items.push({ key: "concierge", label: t("amenityConcierge"), checked: concierge, set: setConcierge });
    if (category === "residential")
      items.push({ key: "petsAllowed", label: t("amenityPetsAllowed"), checked: petsAllowed, set: setPetsAllowed });
    if (category === "residential")
      items.push({ key: "doubleGlazing", label: t("amenityDoubleGlazing"), checked: doubleGlazing, set: setDoubleGlazing });
    if (category === "residential")
      items.push({ key: "underfloorHeating", label: t("amenityUnderfloorHeating"), checked: underfloorHeating, set: setUnderfloorHeating });
    return items;
  }, [
    category, propertyType, hasPropertyType,
    balcony, builtInCloset, garden, alarmSystem, homeAutomation,
    gymAccess, parkingAvailable, swimmingPool, loadingAccess,
    displayFrontage, airConditioning, storageArea, roadAccess,
    utilitiesAvailable, zonedUse, waterSource, electricityNearby,
    maidsRoom, studyRoom, laundryRoom, centralAC, elevator,
    meetingRoom, receptionArea, cctvSecurity, fencedBoundary,
    irrigationSystem, concierge, petsAllowed, doubleGlazing, underfloorHeating,
    t, isResType, isComType,
  ]);
  ...
  const areaOptions = city ? getAreasByCityName(city).map((a) => ({ value: a, label: a })) : [];
  const propertyTypeOptions = CATEGORY_TO_PROPERTY_TYPES[category].map((pt) => ({
    value: pt,
    label: pt,
  }));
    ...
    <div className="space-y-6 pb-28">
      ...
      >
        <div className="grid gap-5 sm:grid-cols-2">
          {/* Listing Purpose */}
          <PropertyFormField label={t("listingPurpose")} htmlFor="listing-purpose" required>
            <Select
              id="listing-purpose"
              value={listingPurpose}
              onChange={(e) => setListingPurpose(e.target.value as StatusTabKey)}
              options={[
          ...
          {/* Category */}
          <PropertyFormField label={t("category")} htmlFor="category" required>
            <Select
              id="category"
              value={category}
          ...
          {/* Property Type */}
          <PropertyFormField label={t("propertyType")} htmlFor="property-type" required>
            <Select
              id="property-type"
              value={propertyType}
          ...
          <div className="flex items-center gap-2 sm:col-span-2">
            <Checkbox
              id="is-exclusive"
              checked={isExclusive}
              onChange={(e) => setIsExclusive((e.target as HTMLInputElement).checked)}
            />
            <label htmlFor="is-exclusive" className="text-size-sm fw-medium text-charcoal cursor-pointer">
              {t("fieldExclusive")}
            </label>
          </div>
        </div>
        ...
        {/* Title */}
        <PropertyFormField label={t("propertyTitle")} htmlFor="property-title" required>
          <Input
            id="property-title"
            value={title}
        ...
        {/* Description */}
        <PropertyFormField label={t("propertyDescription")} htmlFor="property-description">
          <Textarea
      ...
      >
        ...
        {showGovFields && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <PropertyFormField label={t("fieldGovernorate")} htmlFor="governorate">
              <Input
                id="governorate"
                value={governorate}
            ...
            <PropertyFormField label={t("fieldDirectorate")} htmlFor="directorate">
              <Input
                id="directorate"
                value={directorate}
        ...
        {showParcelField && (
          <PropertyFormField label={t("fieldParcelName")} htmlFor="parcel-name">
            <Input
              id="parcel-name"
              value={parcelName}
      ...
      >
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {showBathrooms && (
            <PropertyFormField label={t("fieldBathrooms")} htmlFor="bathrooms">
              <Select
                id="bathrooms"
                value={bathrooms}
          ...
          {showAreaRange && (
            <PropertyFormField label={t("fieldBuiltUpArea")} htmlFor="built-up-area">
              <Input
                id="built-up-area"
                type="number"
                min={0}
                ...
                rightAdornment={<span className="text-xs text-charcoal/50">sq ft</span>}
              />
            </PropertyFormField>
          )}
          ...
          {showFloorLevel && (
            <PropertyFormField label={t("fieldFloorLevel")} htmlFor="floor-level">
              <Select
                id="floor-level"
                value={floorLevel}
                ...
                options={FLOOR_OPTIONS.map((v) => ({
                  value: v,
                  label: v === "ground" ? "Ground" : v === "penthouse" ? "Penthouse" : `Floor ${v}`,
                }))}
          ...
          {showParking && (
            <PropertyFormField label={t("fieldParking")} htmlFor="parking">
              <Select
                ...
                options={PARKING_OPTIONS.map((v) => ({ value: v, label: v === "0" ? "None" : v }))}
          ...
          {showFurniture && (
            <PropertyFormField label={t("fieldFurnitureStatus")} htmlFor="furniture-status">
              <Select
                id="furniture-status"
          ...
          {showPropertyAge && (
            <PropertyFormField label={t("fieldPropertyAge")} htmlFor="property-age">
              <Select
                id="property-age"
                value={propertyAge}
          ...
          <PropertyFormField label={t("fieldTotalFloors")} htmlFor="total-floors">
            <Input
              id="total-floors"
              type="number"
              min={1}
          ...
          <PropertyFormField label={t("fieldCompletionStatus")} htmlFor="completion-status">
            <Select
          ...
          {completionStatus !== "ready" && (
            <PropertyFormField label={t("fieldHandoverDate")} htmlFor="handover-date">
              <Input
                id="handover-date"
                type="date"
                value={handoverDate}
          ...
          <PropertyFormField label={t("fieldReferenceNumber")} htmlFor="reference-number">
            <Input
              id="reference-number"
          ...
          <PropertyFormField label={t("fieldPermitNumber")} htmlFor="permit-number">
            <Input
              id="permit-number"
              value={permitNumber}
          ...
          <PropertyFormField label={t("fieldOrientation")} htmlFor="orientation">
            <Select
              id="orientation"
              value={orientation}
              onChange={(e) => setOrientation(e.target.value)}
              options={ORIENTATION_OPTIONS.map((v) => ({ value: v, label: v }))}
      ...
      >
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {isRental ? (
            <>
              <PropertyFormField label={t("fieldRentFrequency")} htmlFor="rent-frequency" required>
                <Select
                  id="rent-frequency"
                  value={rentFrequency}
              ...
              <PropertyFormField label={t("fieldRentPrice")} htmlFor="rent-price" required>
                <Input
                  id="rent-price"
                  type="number"
                  min={0}
                  ...
                  rightAdornment={
                    <span className="text-xs text-charcoal/50">
                      JOD / {rentFrequency === "monthly" ? t("monthShortcut") : t("yearShortcut")}
                    </span>
                  }
                />
              </PropertyFormField>
            ...
            <>
              <PropertyFormField label={t("fieldPrice")} htmlFor="price" required>
                <Input
                  id="price"
                  type="number"
                  min={0}
                  ...
                  placeholder="0"
                  rightAdornment={<span className="text-xs text-charcoal/50">JOD</span>}
                />
              </PropertyFormField>

              <PropertyFormField label={t("fieldServiceCharge")} htmlFor="service-charge">
                <Input
                  id="service-charge"
                  type="number"
                  min={0}
              ...
              <PropertyFormField label={t("fieldMaintenanceFee")} htmlFor="maintenance-fee">
                <Input
                  id="maintenance-fee"
                  type="number"
                  min={0}
            ...
            {amenityItems.map((item) => (
              ...
              >
                <Checkbox
                  ...
                  onChange={(e) => item.set((e.target as HTMLInputElement).checked)}
                />
                <span className="text-size-sm fw-medium text-charcoal/85 group-hover:text-charcoal transition-colors">
                  {item.label}
                </span>
              </label>
            ))}
      ...
      {/* ============================================================ */}
      {/*  SECTION 7 — Media (Images, Videos, Virtual Tour)             */}
      {/* ============================================================ */}
      <PropertyFormSection
      ...
      >
        <div className="space-y-8">
          {/* 1. Property Images — with preview grid */}
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 text-primary">
                <ImageIcon className="h-4 w-4" />
              </div>
              <h4 className="text-size-sm fw-semibold text-charcoal">{t("mediaSubsectionImages")}</h4>
            </div>
            <MediaUploadField
          ...
          {/* 2. Property Videos — with preview grid */}
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-600">
                <Video className="h-4 w-4" />
              </div>
              <h4 className="text-size-sm fw-semibold text-charcoal">{t("mediaSubsectionVideos")}</h4>
            </div>
            <MediaUploadField
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-size-sm fw-semibold text-charcoal">{t("mediaSubsectionYoutube")}</h4>
            </div>
            <PropertyFormField label={t("fieldYoutubeUrl")} htmlFor="youtube-url">
              <Input
                id="youtube-url"
                type="url"
                value={youtubeUrl}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-size-sm fw-semibold text-charcoal">{t("mediaSubsectionVirtualTour")}</h4>
            </div>
            <PropertyFormField label={t("fieldVirtualTourUrl")} htmlFor="virtual-tour-url">
              <Input
                id="virtual-tour-url"
                type="url"
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-subtle bg-white/95 backdrop-blur-sm shadow-[0_-4px_16px_rgba(15,23,42,0.06)]">
        <div className="container mx-auto flex items-center justify-end gap-3 px-4 py-3 md:px-8">
