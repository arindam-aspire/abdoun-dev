...
/**
 * Media upload with grid previews for images and videos.
 * Shows thumbnails/previews and revokes object URLs on cleanup.
 */
export function MediaUploadField({
  ...
  // Create object URLs for new files, revoke only for removed files; avoids race where preview sometimes doesn't appear
  useEffect(() => {
    ...
    // Create URLs only for new files that don't have one yet (image/video only)
    files.forEach((f) => {
      if (
        !current[f.id] &&
        (IMAGE_TYPES.test(f.file.type) || VIDEO_TYPES.test(f.file.type))
      ) {
  ...
  return (
    <div className="rounded-2xl border border-subtle bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        ...
        >
          <div
            className={`flex h-12 w-12 items-center justify-center rounded-full transition-colors ${
              isDragOver ? "bg-primary/15 text-primary" : "bg-zinc-100 text-zinc-400 group-hover:bg-primary/10 group-hover:text-primary"
            }`}
          >
        ...
        {files.length > 0 && (
          <div>
            <p className="text-size-xs fw-medium text-charcoal/70 mb-3">
