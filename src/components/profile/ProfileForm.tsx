...
export function ProfileForm({
  ...
  const [form, setForm] = useState<Partial<ProfileData> & { avatarPreview?: string }>({});
  ...
  const handlePhotoSelect = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = () => {
        ...
        setForm((prev) => ({ ...prev, avatarPreview: dataUrl, avatarUrl: dataUrl }));
      };
  ...
  return (
    <form
      ...
      dir={isRtl ? "rtl" : "ltr"}
    >
      <div className={cn(compact && "min-h-0 flex-1 overflow-y-auto", "px-4 py-5 md:px-6")}>
